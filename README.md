...
| 型態 | 定義 | 總位寬 | 整數位 | 小數位 | 用途 |
|------|------|--------|--------|--------|------|
| `data_t` | `ap_fixed<32, 16>` | 32-bit | 16 | 16 | 輸入/輸出訊號 (Q16.16) |
| `accum_t` | `ap_fixed<80, 32>` | **80-bit** | 32 | 48 | Gram 矩陣累加器 |
| `coef_t` | `ap_fixed<32, 16>` | 32-bit | 16 | 16 | 諧波係數 α, β |
| `trig_t` | `ap_fixed<24, 2>` | 24-bit | 2 | 22 | sin/cos 值 (範圍 ±2) |
| `phase_t` | `ap_fixed<32, 4>` | 32-bit | 4 | 28 | 相位 (高精度小數) |
| `freq_t` | `ap_fixed<40, 16>` | 40-bit | 16 | 24 | DBS 頻率 |
| `timestamp_t` | `ap_fixed<40, 12>` | 40-bit | 12 | 28 | 時間戳 |
| `omega_t` | `ap_fixed<40, 12>` | 40-bit | 12 | 28 | 角頻率 (2πf) |
| `axis_pkt_t` | `ap_axis<32, 0, 0, 0>` | 32-bit | - | - | AXI-Stream 封包 |

### 為何 accum_t 需要 80-bit？

```
Gram 矩陣累加：M[i][j] = Σ (trig × trig)，n = 0 到 N-1

單次乘法：24-bit × 24-bit = 48-bit
累加次數：N = 3696（視窗大小）
累加增長：log₂(3696) ≈ 12-bit
